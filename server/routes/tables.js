...
router.post("/", async (req, res) => {
  try {
    ...
    if (table) return res.status(409).send({ message: "already Exist" });
...
router.put("/:tableId", async (req, res) => {
  try {
    ...
    if (tableId && tableStatus) {
      ...
      res
        .status(200)
        .json({ data: table, message: "Table status updated successfully" });
    }
    if (tableId && x && y) {
      const table = await Table.findByIdAndUpdate(
        tableId,
      ...
      res
        .status(200)
        .json({ data: table, message: "Table position updated successfully" });
    }
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
...
router.delete("/:tableId", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ data: table, message: "Table deleted successfully" });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
});
