...
const UserPermissions = ({ handleLogout }) => {
  ...
  const deleteUser = async (userId, userEmail, userRole) => {
    ...
    if (userRole === "Employee") {
      ...
      confirmed = window.confirm(
        "Czy na pewno chcesz usunąć konto? Uwaga usuwasz też pracownika"
      );
    } else {
    ...
    if (confirmed) {
      if (userId) {
        try {
          ...
          if (userRole === "Employee") {
            //delete user with role empl -> delete user & empl
            setIsEmployee(true);
    ...
    if (confirmed) {
      if (employeeId) {
        try {
          ...
          if (control === true) {
            //delete empl -> change roles empl->user
  ...
  const modifyUser = async (userId, modifyUser) => {
    console.log(userId);
    try {
      if (modifyUser.roles === "User") {
        //if roles empl->user = delete from empl
        const orginalUser = users.find((user) => user._id === userId);
        if (orginalUser && orginalUser.roles === "Employee") {
          const findEmployee = employees.find(
            (employee) => employee.email === orginalUser.email
          );
          deleteEmployee(findEmployee._id);
        }
      } else if (modifyUser.roles === "Employee") {
        //from user role to empl
        const orginalUser = users.find((user) => user._id === userId);
  ...
  const addEmployee = async (dataPerson) => {
    try {
      const { password, roles, _id, __v, points, ...dataWithoutPassword } =
        dataPerson; //usuwanie danych używanych dla kolekcji user które nie mają zastosowania w empl
                      ...
                      <tbody>
                        {users.map((user) => (
                          <tr key={user._id}>
                            ...
                            <td>
                              <button
                                className={`${styles.edit_btn} link_btn`}
                                onClick={() => {
                            ...
                            <td>
                              <button
                                className={`${styles.link_btn} link_btn`}
                                onClick={() =>
                                  deleteUser(user._id, user.email, user.roles)
                                }
                              >
                                Usuń konto
                              </button>
